...
<div class="container mt-3">
  ...
  <table class="table table-hover">
    ...
    <tbody>
        ...
        $ch = curl_init(); //$data=file_get_contents($url);
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Kembalikan hasil sebagai string
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            "Accept: application/json" // Pastikan kita meminta data dalam format JSON
        ]);
        curl_setopt($ch, CURLOPT_TIMEOUT, 10); // Atur waktu tunggu maksimum 10 detik
